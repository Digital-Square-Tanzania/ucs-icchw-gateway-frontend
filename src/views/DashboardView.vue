...
<template>
  ...
  <div id="dash-top" class="lg:flex lg:flex-row md:flex md:flex-col md:mt-4 sm:grid sm:grid-cols-1 lg:gap-0 sm:gap-2">
    <div id="left-cards"
      class="mt-5 flex flex-col gap-5 lg:grid lg:grid-cols-2 lg:w-1/2 lg:pr-2 lg:mt-0 lg:gap-4 md:grid md:grid-cols-2 md:w-full md:mt-0 md:gap-4">

      <InfoCard color="green">
        <template #heading>OpenMRS Users</template>
        <template #big-number>{{ dashboardData ? formatNumber(dashboardData.openMRSUsers) : 'Loading...' }}</template>
      ...
      <InfoCard color="red">
        ...
        <template #big-number>{{ dashboardData ? formatNumber(dashboardData.dhis2Users) : 'Loading...' }}</template>
        <template #percent>{{ openMRSUsersRatio ? Math.ceil(100 - openMRSUsersRatio) + '%' : 'Loading...' }}</template>
        <template #subtext>Last Sync: {{ lastSynced.dhis2 || 'N/A' }}</template>
        <template #sync>
          <a href="#" @click.prevent="startSync('dhis2')"
            class="text-ucs-500 hover:text-ucs-800 dark:text-ucs-400 dark:hover:text-ucs-200">
            {{ syncStatus.dhis2 === 'syncing' ? 'Syncing...' : 'Sync DHIS2' }}
          </a>
        </template>
        <template #icon>
      ...
      <InfoCard color="red">
        ...
        <template #subtext>Last Sync: {{ lastSynced.facilities || 'N/A' }}</template>
        <template #sync>
          <a href="#" @click.prevent="startSync('facilities')"
            class="text-ucs-500 hover:text-ucs-800 dark:text-ucs-400 dark:hover:text-ucs-200">
            {{ syncStatus.facilities === 'syncing' ? 'Syncing...' : 'Sync OpenMRS' }}
          </a>
        </template>
        <template #icon>
    ...
    <div id="right-cards" class="lg:w-1/2 lg:pl-2 md:w-full">
      <div
        ...
        <BigCard>
          ...
          <template #big-number>{{ dashboardData ? formatNumber(dashboardData.ucsTeams) : 'Loading...' }}</template>
  ...
  <div id="dash-bottom" class="flex flex-col lg:flex-row gap-4 pt-4">
